...
ids_January_6 <- as.numeric(scan("Trump-rally-ids.txt", character(), quote = ''))
january6 <- tibble()

for (i in 1:1721){
  jan_temp <- lookup_tweets(ids_January_6[(i-1)*870+1:870*i])
  jan_temp <- jan_temp[, -which(names(jan_temp) %in% c("coordinates", "retweeted_status", "quoted_status_permalink", "quoted_status", "geo", 'place', "entities", "possibly_sensitive", "truncated", "contributors", "favorited", "favorited_by", "scopes", "display_text_width", "quote_count", "timestamp_ms", "reply_count", "filter_level", "metadata", "query", "withheld_scope", "withheld_copyright", "withheld_in_countries", "possibly_sensitive_appealable"))]
  january6 <- rbind(january6, jan_temp)
  write.csv(january6, "january6.csv")
  print(i)
}
